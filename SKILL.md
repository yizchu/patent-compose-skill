...
description: AI驱动的专利撰写工作流，自动化完成项目分析、查新检索、专利组合生成与保护优化、交底书和附图生成、权利要求书及完整申请文件生成。
user-invocable: true
allowed-tools: Read, Write, Edit, Grep, Glob, WebSearch, RunCommand
---

## 概述
本 Skill 用于辅助完成专利撰写全流程，包含**七个阶段**：项目分析、市场分析、项目成果查新、专利组合生成与权利要求树优化、交底书与附图生成、权利要求书撰写、完整专利申请文件（说明书+摘要）生成。
...
**功能**：分析项目源码和文档，提取核心技术方案和潜在专利点

**输出**：
- `materials/disclosure-v1.md` - 第一版技术交底书，是项目的分析结果
- `materials/formula_inventory.md` - 项目中使用的公式清单

---

### Stage 2: 市场分析 → 初始权利要求树

**`Read`** `${SKILL_DIR}/prompts/stage2_analyze_market.md`

**功能**：基于项目分析结果，结合产品规划、竞品分析和未来市场需求预判，生成初版权利要求树和市场分析报告

**输出**：
- `materials/market-report.md` - 市场分析报告（轻量版或深度版）
- `materials/claim-tree-v1.json` - 初版权利要求树（JSON格式）

---

### Stage 3: 项目成果查新

**`Read`** `${SKILL_DIR}/prompts/stage3_prior_search.md`
...
### Stage 4: 专利组合生成与权利要求树优化

**`Read`** `${SKILL_DIR}/prompts/stage4_design_portfolio.md`

**功能**：布局专利组合，并通过六轮博弈对抗优化

**输出**：
- `materials/portfolio-initial.json` - 初始专利组合方案
- `materials/portfolio-initial.md` - 初始专利组合方案的生成报告
- `materials/portfolio-v2.json` - 最终专利组合方案
- `claim-optimization/` - 六轮攻防记录（R1-R6.json）+ HTML 可视化报告

### Stage 5: 撰写最终交底书与附图

**`Read`** `${SKILL_DIR}/prompts/stage5_generate_disclosure.md`

**功能**：为每件专利撰写完整的技术交底书（融合查新成果）并绘制 Mermaid 附图

**输出**：
- `patents/disclosure_{专利标题}.md` - 各专利独立交底书（最终版）
- `patents/flowcharts_{专利标题}.md` - 各专利附图（Mermaid格式）
- `materials/disclosure-v2.md` - 汇总版最终交底书

### Stage 6: 撰写权利要求书

**`Read`** `${SKILL_DIR}/prompts/stage6_compose_claim.md`
...
**`Read`** `${SKILL_DIR}/prompts/stage7_compose_application.md`
...
| `prior_search.py` | 专利数据库检索与查新分析（支持CNKI/FPO/WebSearch） | Stage 3 |
| `generate_optimization_html.py` | 六轮博弈对抗优化报告生成（HTML可视化） | Stage 4 |
| `claim.py` | 权利要求树转权利要求书文本，格式验证 |  |
| `yaml_to_json.py` | YAML格式转换为JSON（用于生成JSON文件） |  |
| `file_tools.py` | 文件读写、JSON 格式验证与自动修复 | 通用工具 |
| `config.py` | 全局配置（输出目录、缩进等） | 通用配置 |
| `network_tools.py` | 网络请求工具 | 通用工具 |