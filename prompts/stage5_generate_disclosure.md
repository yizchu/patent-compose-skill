...
基于 Stage 1 的项目分析结果、Stage 3 的查新报告、以及 Stage 4 的专利组合方案和优化后的权利要求树，为每件专利撰写完整的**技术交底书**，并绘制附图。
...
2. **`${输出目录}/materials/prior-art-report.md`**：Stage 3 生成的查新报告（包含技术特征界定和创造性评估）
3. **`${输出目录}/materials/portfolio-v2.json`**：Stage 4 生成的优化后专利组合（包含最终的权利要求树）
4. **`${输出目录}/materials/formula_inventory.md`**：Stage 1 生成的公式清单（**必须读取**，包含所有识别到的数学公式）

---

### Step 2：改进交底书

基于 Stage 1 的原始分析结果、Stage 3 的查新报告、以及 Stage 4 优化后的权利要求树，对交底书进行全面优化。
...
## 七、核心公式与算法（重要）
> 要求：详细列出本专利涉及的所有关键数学公式、算法表达式和计算模型。这是专利技术方案的核心支撑，必须在交底书中明确。

### 7.1 公式总览

基于 Stage 1 识别的公式清单（`formula_inventory.md`），本专利共涉及 **{N}** 个核心公式，按相关性分级如下：

**🔴 高相关性公式（必须在说明书中详细描述）**：{M} 个
- [公式1名称]（{formula_id}）：[一句话概括作用]
- [公式2名称]（{formula_id}）：[一句话概括作用]

**🟡 中相关性公式（建议在实施例中提及）**：{K} 个
- [公式3名称]（{formula_id}）：[一句话概括作用]

### 7.2 核心公式详解

对每个**高相关性**公式，按以下模板详细描述：

#### 公式 {编号}：[公式名称]
**公式编号**：{formula_id}
**原始出处**：{original_source}
**技术作用**：[该公式在整体技术方案中的作用，解决了什么问题]
...
| 符号 | 名称 | 类型 | 取值范围 | 说明 |
|------|------|------|---------|------|
| {symbol1} | {name1} | {type1} | {range1} | {description1} |
| {symbol2} | {name2} | {type2} | {range2} | {description2} |
| ... | ... | ... | ... | ... |
...
**计算过程**：
{step-by-step calculation showing intermediate results}
...
| 参数 | 推荐取值 | 取值范围 | 调优建议 |
|------|---------|---------|---------|
| {param1} | {rec_value1} | {range1} | {tuning_tip1} |
| {param2} | {rec_value2} | {range2} | {tuning_tip2} |

**与其他模块的关系**：
[说明该公式的输入来自哪个模块/步骤，输出提供给哪个模块/步骤使用]
...
**用途**：[简述该公式的作用和使用场景]
**关键参数**：[列出重要参数及其推荐值]
**详见实施例**：在实施例{X}中有具体应用示例

（如有更多中相关性公式，按相同格式继续列出）
...
## 八、具体实施方式
> 要求：结合附图，详细描述实现发明的具体方式，使本领域技术人员能够再现。应结合具体的实施例进行说明。
...
1. **读取规范**：首先使用 `Read` 工具读取 `${SKILL_DIR}/prompts/references/chart_drawing_specifications.md` 文件，熟悉图表绘制规范。
...
提醒用户检查交底书和附图是否符合预期，确认无误后Stage5才算完成。如果用户有异议，需要结合用户反馈和已有分析结果进行修正，直到用户满意为止。完成后主动提示用户："Stage5已完成，如果会话上下文接近上限，建议在新会话中继续后面的步骤，并请告知智能体已完成Stage5"。
